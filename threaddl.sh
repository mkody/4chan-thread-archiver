...
usage () {
	...
	echo ""
	echo "Usage: $name [4chan thread URL] <time value and/or thread name>"
	echo ""
	echo "  <time value>"
	...
	echo ""
	echo "  <thread name>"
	echo "     Set the thread name to add to the end of the URL"
	echo "     Default: none"
	exit 0
}
...
LOC=$(echo "$URL" | sed 's_.*/\([^/]\+\)/thread/\([0-9]\+\)_\1\-\2_')
...
thejob () {
	...
	egrep "//${ST}/image/[^.]+\...." $LOC.html -o | sed 's_^//_http:&_g' | uniq >> ./misc/misc

	egrep "//${ST}/image/country/[^.]+\...." $LOC.html -o | sed 's_^//_http:&_g' >> $LOC/misc/misc

	egrep "//${ST}/css/[a-z]+\.[0-9]+\.css" $LOC.html -o | sed -e 's_^//_http:&_' | head -n1 > ./misc/css

	egrep "//${ST}/css/[a-z]+\.[0-9]+\.css" $LOC.html -o | sed -e 's_^//_http:&_' | egrep 'tomorrow|prettify' >> ./misc/css
...
EOF
	...
	sed -i -e '1 {s_<script_!>\n&_
		s_<link [^>]*RSS feed[^>]*>__
		s@//'$ST'/image/\(favicon[^.]*\.ico\)@/4chan/misc/\1@
		s_<link rel="alternate style[^-]*\(<link[^>]*tomorrow\.[^>]*>\)<link[^>]*>_\1_
		s@//'$ST'/css/\([^.]\+\.[^.]\+\.css\)@http://s.4cdn.org/css/\1@g}' -e '$ {s_</head>_<!&_
		...
		s@//'$ST'/image/title/[a-z0-9-]*\.\(...\)@/4chan/misc/logo.php@g
		...
		s_<div class="mFileInfo mobile">[^<]*</div>__g}' $LOC.html
